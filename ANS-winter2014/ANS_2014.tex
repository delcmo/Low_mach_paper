...
RELAP-7, the next-generation reactor safety system code built upon the MOOSE multiphysics framework
...
The Euler equation with viscous stabilization are discretized with {\it continuous} finite elements in space and BDF2 in time using MOOSE. The resulting nonlinear system of equations at each time step is solved using a Jacobian-free Newton Krylov technique. We present results for a 2D flow over a circular hump \cite{Hump}. Such problems are often employ to test the ability of numerical schemes used in compressible flow solvers to resolve low-Mach flows (i.e., in the incompressible limit) \cite{LowMach1}. 
