...
\begin{block}{Other RELAP-7 talks}
\hspace{0.5cm} $\bullet$ 
\tcb{Tuesday Nov. 11, 10:10 AM:} {\it Status Update on RELAP-7: A Next Generation Reactor System
Analysis Code},
Richard Martineau, Hongbin Zhang, Haihua Zhao,
Ling Zou (INL) \\
\hspace{0.5cm} $\bullet$ 
\tcb{Wednesday Nov. 12, 8:30 AM:} {\it Application of the Entropy Viscosity Method to the 1-D 7-Equation
Model for Two-Phase Flows}, 
Marc O. Delchini, Jean C. Ragusa (TAMU), Ray A. Berry (INL)\\
%\hspace{0.5cm} $\bullet$ 
...
\hspace{0.9cm} $\to$ Nonlinear system of equations \\%(preconditioner: pressure-correction ICE, for example)\\
\hspace{0.9cm} $\to$ Fits the JFNK formalism in MOOSE % where all physic components are tightly coupled \\
\end{block}

\begin{block}{Viscous regularization of the discretized fluid flow equations}
We will employ novel \textcolor{magenta}{artificial viscosity} schemes based on the entropy production residual
