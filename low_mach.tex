...
The derivation of the viscous regularization (or dissipative terms) is carried out to be consistent with the entropy minimum principle theorem \tcr{is this really a theorem?}; details and proofs of the derivation can be found in \cite{jlg}. The viscous regularization thus obtained is valid for any equation of state as long as the physical entropy function $s$ is such that $-s$ is a convex function with respect to the internal energy $e$ and the specific volume $1/\rho$. Euler equations with viscous regularization are given in \eqt{eq:euler_visc}:
%
\begin{subequations}
\label{eq:euler_visc}
%
...
Other stabilization approaches have been proposed in the literature, for instance, the Lapidus \cite{Lapidus_book, Lapidus_paper} or the pressure-based viscosity methods \cite{PBV_book}. Here, we follow the work of Guermond et al. and define the viscosity coefficients, $\kappa$ and $\mu$, based on the local entropy production. These coefficients are numerically evaluated using the local entropy residual $\resi(\vec{r},t)$ defined in \eqt{eq:ent_residual}; $\resi(\vec{r},t)$ is known to be peaked in shocks and vanishingly small elsewhere \cite{Toro}. 
%
\begin{equation}
...
The denominator $|| s - \bar{s} ||_\infty$ is used for dimensionality purposes \tcr{and should not be of the same order as $h$, on penalty of loosing the high-order accuracy} \tcb{you do not like this?}. Currently, there are no theoretical justification for choosing the denominator beyond a dimensionality argument. 
...
\item other suitable normalizations for the residual $\resinew$ can be devised. Examples include the pressure itself or combinations of the density, the speed of sound and the norm of the velocity, i.e., $\rho c^2$, $\rho c || \vec{u} ||$ or $\rho || \vec{u} ||^2$. 
\end{itemize}
%
Denoting the normalization of $\resinew$ by $\norm_P$, the entropy-based viscosity coefficients $\mu_e$ and $\kappa_e$ can be re-defined using as follows:
%
\begin{subequations}
...
\tcb{What about: Derivation of the all-speed reformulation of the viscosity coefficients}\tcr{ Let's talk}
%===================================================================================================
...
\end{equation}
%
It is also observed that the Prandlt number is a function of the Reynolds and P\'eclet numbers as follows: 
\begin{equation} \label{eq:ref_nb_pr} 
\Pr_\infty = \Pe_\infty / \Re_\infty 
\end{equation}
For simplicity, we use here the ideal gas equation of state. Its dimensionless expression is given by
%
\begin{equation}
...
%
The numerical Reynolds and P\'eclet numbers defined in \eqt{eq:ref_numb} are related to the entropy viscosity coefficients. Thus, once an appropriate scaling is obtained for each of the non-dimensionalized numbers, the corresponding normalization parameters $\norm_P^\mu$ and $\norm_P^\kappa$ will be derived. 
...
Our objective is to determine the appropriate scaling for the non-dimensionalized numbers, $\Re_\infty$ and $\Pe_\infty$, in the two previous situation so that shocks are stabilized and the isentropic incompressible limit and its main features are preserved.
...
\end{subequations}
%
Using the same scaling, $\Re_\infty = \Pe_\infty = 1$, the continuity, momentum and energy equations at order $1$ yield:
...
\end{equation}
%
Therefore, we conclude that by setting the non-dimensionalized Reynolds and P\'eclet numbers to one, the incompressible fluid equations are retrieved when performing a low Mach asymptotic limit of the compressible Euler equations. Plus, the scaling of the Prandlt number can be derived using \eqt{eq:ref_nb_pr} and found equal to one.\\

We now consider the non-isentropic case. Even subsonic flows can present shocks (for instance, a step initial condition in the pressure will trigger shock formation, independently of the Mach number). The non-dimensional form of the Euler equations given in \eqt{eq:Euler_eq2} provides some insight on the dominant terms in the low-Mach limit. This is particular obvious in the momentum equation, \eqt{eq:euler_eq2_mom}, where the gradient of pressure is multiplied by $1/M^2$. This pressure gradient term may need to be stabilized by some dissipative terms of the same scaling to prevent spurious oscillations from forming when shocks develop. This leads to the following three possible requirements regarding the non-dimensionalized Reynolds and P\'eclet numbers: 
(a) $\Re_\infty = M_\infty^2$ and $\Pe_\infty = 1$,
(b) $\Re_\infty = 1$ and $\Pe_\infty = M_\infty^2$, or
(c) $\Re_\infty = \Pe_\infty = M_\infty^2$. 
%
Any of the previous choices will also affect the stabilization of the continuity and energy equations. For instance, setting P\'eclet number equal to $M_\infty^2$ will efficiently stabilize the continuity equation in the shock region but will also add an excessive amount of dissipation in the contact wave since dissipating on the density. Such a behavior is not suitable for accuracy purpose, making options (b) and (c) invalid and leaving us with option (a). The same reasoning, left to the reader, can be carried for the energy equation (\eqt{eq:euler_eq2_energy}) leading to the same conclusion. With option (a), all of the dissipative terms in the continuity and energy equations are well-scaled. Thus, it is chosen to use option (a) to solve the non-isentropic form of the Euler equations. Under this assumption, the scaling of the Prandlt number can be derived using \eqt{eq:ref_nb_pr}: $Pr_{\infty} = M_{\infty}^{-2}$.
%
%Before performing a low-Mach asymptotic limit, it is proposed to determine the scaling of the Prandlt number. It can be shown that the P\'echlet number is a function of the Reynolds and Prandlt number as follows:$Pe_\infty = Re_\infty Pr_\infty$. Using the condition imposed by (a) and the previous relation, it is easily derived that the Prandlt number scales as the inverse of the Mach number: $Pr_\infty = M_\infty^2$.
%
...
The study of the two limit cases yields two different scalings of the non-dimensionalized Peclet number: $\Pe_\infty = 1$ in the isentropic case and $\Pe_\infty  = M_\infty^2$ for non-isentropic compressible Euler equations, whereas the numerical Reynolds number is always equal to one. In order to have a stabilization method valid for a wide range of Mach numbers, including situations with shocks, these two scalings should be merged into a unique representation. An option is to investigate how the non-dimensionalized entropy residual, $\resinew^*$, scales for different flow types (isentropic and non-isentropic). For an isentropic flow, i.e. without shocks, the non-dimensionalized entropy residual is known to scale as the Mach number. On the other hand, in the shock region, the non-dimensionalized entropy residual will experience a peak. Thus, by investigating the local variation of the non-dimensionalized entropy residual, the flow type can be deduced and the proper normalization used, as follows:
\begin{eqnarray}  \label{eq:norm_ent}
&&\text{if } \left| \resinew^* \right| \geq M \text{ (non-isentropic flow)}\nonumber \\
&&\Pe_\infty = M^2 \nonumber \\
&&\text{else (isentropic flow)}\\
&&\Pe_\infty = 1 \nonumber
\end{eqnarray}
where $M$ is the \emph{local Mach number}. 

Once we have deduced the scaling of the numerical non-dimensionalized number, the normalization parameters $n_P^{\mu}$ and $n_P^{\kappa}$ can be derived. Only the steps leading to the derivation of $n_P^{\kappa}$ for the viscosity coefficient $\kappa$ are detailed. Using the definition of the viscosity coefficients given in \eqt{eq:visc_definition} and the scaling of \eqt{eq:norm_param}, it can be shown that:
%
\begin{equation}
\label{eq:norm_relation}
\kappa_\infty = \frac{ \rho_\infty c_\infty^2 u_\infty L }{ n_{P,\infty}^{\kappa} } 
\end{equation}
%
where $n_{P,\infty}$ is the reference quantity for the normalization parameter $n_P$. Substituting \eqt{eq:norm_relation} into \eqt{eq:ref_numb} and recalling that the numerical Prandlt number scales as unity, it yields:
%
\begin{equation}
\label{eq:norm_relation_bis}
n_{P,\infty}^{\kappa} = \rho_\infty c_\infty^2 .
\end{equation}
%
\eqt{eq:norm_relation_bis} tells us that in the asymptotic limit, the normalization parameter $n_P^\kappa$ scales as $\rho_\infty c_\infty^2$ which leaves us with two options:
either $n_P = \rho c^2$ or $n_P = P$. The choice was made to use $n_P^{\kappa} = \rho c^2$ in the low-Mach limit: it was found to behave well and the pressure can become locally negative and null in some particular case as shown in \sct{sec:results}. \\
On the same model, the normalization parameter $\norm_P^{\mu}$ for the viscosity coefficient $\mu$ is derived for the two cases given in \eqt{eq:norm_ent}:
\begin{eqnarray}  \label{eq:norm_ent2}
&&\text{if } \left| \resinew^* \right| \geq M \text{ (non-isentropic flow)}\nonumber \\
&&n_{P}^{\mu} = \rho ||\vec{u} ||^2 \nonumber \\
&&\text{else (isentropic flow)}\\
&&n_{P}^{\mu} = \rho c^2 = n_{P}^{\kappa} \nonumber
\end{eqnarray}
...
\text{where } \mu_{\max}(\vec{r},t) = \frac{h}{2} \Big ( ||\vec{u}|| + c \Big ) \\

\mu_{e}(\vec{r},t) = h^2 \max(\resinew, J) / \rho c^2 \\
\kappa_{e}(\vec{r},t) = h^2 \max(\resinew, J) / n_P^{\kappa} \\
J = || \vec{u} || \max \Big ( [[ \grad P \cdot \vec{n} ]], c^2 [[\grad \rho \cdot \vec{n}]] \Big) 
\end{array}
\right.
\end{equation}
where $n_P^{\kappa}$ is computed from \eqt{eq:norm_ent2}. The jump $J$ is a function of the jump of pressure and density gradients across the face with respect to the its normal vector $\vec{n}$. Then, the largest value over all faces is determined and used in the definition of the viscosity coefficients.

With the definition of the viscosity coefficients $\mu$ and $\kappa$ proposed in \eqt{eq:final_def_visc_coeff}, the low Mach asymptotic limit is ensured for isentropic flow, and flow with shocks should be correctly resolved. The viscosity coefficients are only function of local variables and thus should accurately resolved complex flow where both incompressible and compressible regimes coexist. As the flow becomes locally supersonic, the viscosity coefficients $\mu$ and $\kappa$ will be of the same order of magnitude which is consistent with the original definition of the entropy viscosity method recalled in \sct{sec:background} and proposed by Guermond et al. in \cite{jlg1, jlg2}.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%s%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{table}[!htbp]
\begin{center}
\caption{\label{tbl:stff_gas_eos} Stiffened Gas Equation of State parameters for steam and liquid wate.}
...
Lastly, the convergence rates are computed using the following relations 
\begin{equation}\label{eq:conv_rates}
rate = \ln \left( \frac{|| \cdot ||_{2h}-|| \cdot ||_h}{|| \cdot ||_h-|| \cdot ||_{\frac{h}{2}}} \right) / \ln 2
\end{equation}
where $|| \cdot ||$ denotes either the $L1$ or $L2$ error norms, and $h$ is the grid size.
