...
x = \frac{x^*}{L_{\infty}} \text{, } t = \frac{t^*}{t_{\infty}} \text{, } u = \frac{u^*}{u_{\infty}}
\end{eqnarray}
...
\right.
\end{equation}
where the Strouhal $(Str_{\infty})$, the \emph{numerical} Reynolds $(Re_{\infty})$ and Prandtl $(Pr_{\infty})$ numbers are defined as follows:
\begin{eqnarray}
\label{eq:ref_numb}
Str_{\infty} = \frac{L}{u_{\infty} t_{\infty}} \text{ and }
Re_{\infty} = \frac{u_{\infty} L_{\infty}}{\mu_{\infty}} \text{ and }
Pr_{\infty} = \frac{\mu_{\infty}}{\kappa_{\infty}} \text{.}
\end{eqnarray}
The Reynolds and Prandtl numbers are related to the viscosity coefficients $\mu$ and $\kappa$ through the viscosity reference values. The Strouhal number is introduced because of the different scaling between the fluid velocity and the acoustic waves. The objective is to derive the asymptotic equations for both cases and see how the artificial dissipative terms affect the physics. For simplicity purpose, the upper script $^*$ is dropped in the remaining of this section.\\
...
%%
\item Acoustic time scale: $Str_{\infty} = 1 / M_{\infty}$ and $Re_{\infty} = Pr_{\infty} = M_{\infty}$ or $\left\{m, n\right\} = \left\{1, 0 \right\}$:\\
...
\end{itemize}
The normalizations of the Reynolds and Prandtl numbers chose for the fluid $(Str=1)$ and acoustic $(Str=1 / M_{\infty})$ time scales are in conflict. The asymptotic study of the \eqt{eq:euler_visc} for $Str_{\infty} = 1$ and the choice $Re_{\infty} = Pr_{\infty} = 1$ is proved to converge to the incompressible equations in the low Mach limit with the correct order in the pressure and velocity fluctuations at steady-state. However, during a transient, the influence of the acoustic wave cannot be neglected, and in the second asymptotic study for $Str = 1 / M_{\infty}$ and $Re_{\infty} = Pr_{\infty} = M_{\infty}$, the artificial dissipative terms are better scaled to stabilize the acoustic waves. In order to be consistent with the low Mach asymptotic equation at steady-state and be able to resolve low Mach shocks, the two above cases need to be merged into an unique one. To do so, it is proposed to define two high-order viscosity coefficients, $\kappa_{e,1}$ and $\kappa_{e,2}$, as follows (since $m=n$ for both cases which implies $\mu$=$\kappa$, only a definition for $\mu$ is given):
\begin{equation}\label{eq:visc_def_1_and_2}
\left\{
\begin{array}{ll}
\mu_{e,1} = h^2 \frac{\max(\tilde{D_e}, J)}{\rho c^2} \text{ for steady-state}\\
\mu_{e,2} = h^2 \frac{\max(\tilde{D_e}, J)}{\rho c || \vec{u} ||} \text{ for transient and acoustic wave}
\end{array}
\right.
\end{equation}
Then, the two above viscosity coefficients are merged using a weight function that depends on the time derivative of the pressure as follows:
\begin{equation}\label{eq:final_def_visc}
\mu_e = (1 - w(t)) \mu_{e,1} + w(t)\mu_{e,2} \text{ where } w(t) = \min \left( \frac{\Delta t}{P} \partial_{\tau} P, 1 \right)
\end{equation} 
The term $\frac{\Delta t}{P} \partial_{\tau} P$ denotes the time derivative of the pressure with respect to the acoustic time scale $\Delta \tau =  \Delta t /M_{\infty}$. With the definition of the high-order viscosity coefficient $\mu_e$ given in \eqt{eq:final_def_visc}, the viscosity coefficient $\mu_2$ dominates during a transient since $w(t) \rightarrow 1$. On the other hand, at steady-state, $\mu_e \rightarrow \mu_1$ ($w(t) \rightarrow 0$) and the correct asymptotic equations are recovered with pressure fluctuation of the order of the Mach number square. When the fluid becomes supersonic, the fluid velocity $|| \vec{u} ||$ and the speed of sound $c$ are of the same order of magnitude: $|| \vec{u} || \sim c$. When substituting into \eqt{eq:visc_def_1_and_2}, it is noticed that the viscosity coefficients $\mu_{e,1}$ and $\mu_{e,2}$ become equal to each other so that $\mu_e = \mu_{e,1} = \mu_{e,2}$. Thus, the definition of the high-order viscosity coefficient $\mu_e$ given in \eqt{eq:final_def_visc} seems to be valid for a wide range of Mach numbers. \\
%\begin{equation}
%\left\{
%\begin{array}{ll}
...
\text{and } \mu_e = (1 - w(t)) \mu_{e,1} + w(t)\mu_{e,2} \text{ where } w(t) = \min \left( \frac{\Delta t}{P} \partial_{\tau} P, 1 \right)\\
\mu_{e,1} = h^2 \frac{\max(\tilde{D_e}, J)}{\rho c^2} \\
\mu_{e,2} = h^2 \frac{\max(\tilde{D_e}, J)}{\rho c || \vec{u} ||} \\
...
\right.
\end{equation}
The dissipative terms are very similar to the ones obtained for the multi-D Euler equations: each dissipative flux is multiplied by the variable area $A$ in order to  ensure conservation of the flux. When assuming a constant area, \eqt{eq:euler_visc} is retrieved. The derivation only requires the viscosity coefficients $\mu$ and $\kappa$ to be positive. Thus, the viscosity coefficients $\mu$ and $\kappa$ will be set equal to each other and will follow the definition given in \eqt{eq:final_def_visc_coeff}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%---------------------------------------------------------------------------------------------------
A Jacobian-Free-Newton-Krylov (JFNK) method is used to solve for the solution at each time step. The jacobian matrix of the discretized equations was derived by hand, hard coded and used as a preconditioner. This method requires the partial derivative of the pressure with respect to the conservative variables to be known. The contribution of the artificial dissipative terms to the jacobian matrix is simplified by assuming constant viscosity coefficients as shown in \eqt{eq:jac_diss_term} for the dissipative terms of the continuity equation:
\begin{equation}
...
\item The pressure and velocity fluctuations are proportional to the inlet Mach number square and the inlet Mach number at steady-state, respectively, as follows: 
\begin{eqnarray}
