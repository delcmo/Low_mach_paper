...
\nonumber
\end{table}
In \fig{fig:pressure_vel_fluc}, the pressure and velocity fluctuations are plotted (log-log plot) as a function of the far field Mach number. The pressure and velocity fluctuations are expected to be function of the Mach number square and the Mach number, respectively. For reference purpose, the function $f(M) = M^2$ and $f(M)=M$ are plotted. The objective of \fig{fig:pressure_vel_fluc} is to show that the new definition of the viscosity coefficients lead the correct convergence order in the low Mach limit. 
\begin{figure}[H]
\centering
...
\end{eqnarray}
The right hand-side is now integrated by part:
\begin{eqnarray}
...
\begin{equation}
rhs = \div \left[ s_e \tilde{h}-s_e e f  + \rho s_{\rho} f \right] - \div \tilde{h} \grad s_e  - f \grad (e s_e) - f \grad ( \rho s_{\rho} ) \nonumber
\end{equation}
...
\begin{equation}
rhs = \div ( A \kappa \grad s ) - A \kappa \underbrace{\left[ \grad (\rho e) \grad s_e  + \grad \rho \grad (e s_e) +  \grad ( \rho s_{\rho} )  \right]}_{\mathbf{Q}} \nonumber
\end{equation}
