...
%
The asymptotic study presented here is inspired by the work of Muller et al. \cite{Muller} where an asymptotic derivation for the Navier-Stokes was presented. Here, we remind the reader that the objective of the asymptotic study is to determine appropriate scaling for the entropy viscosity coefficients in the \tcr{low-Mach limit} so that the dissipative terms remain well-scaled for two limit cases: 
...
(ii) the non-isentropic limit with formation of shocks. \tcr{item (ii) is not low-Mach per se. need to revised this paragraph\\}
...
\end{equation}
For simplicity, we use here the ideal gas equation of state. Its non-dimension expression is given by
%
\begin{equation}
...
In our low-Mach asymptotic study, we consider the isentropic and non-isentropic situations. The isentropic limit of Euler equations with viscous regularization should yield the incompressible fluid flows results of the low-Mach limit, namely, that the pressure fluctuations are of the order $M^2$ and that the velocity satisfies the divergent constraint $\div \vec{u}_0 = 0$ \cite{LowMach1, LowMach2, LowMach3}. For non-isentropic situations, shocks may form for any value of Mach number (even in subsonic flows) and the minimum entropy principle should still be verified so that spurious oscillations be controlled by the entropy viscosity method, independently of the Mach number value.
Our objective is to determine the appropriate scaling for the non-dimensionalized numbers, $\Re_\infty$ and $\Pe_\infty$, in the two limit-cases\tcr{? so that shocks are stabilized and the isentropic incompressible limit and its main features are preserved.}
...
\end{subequations}
%
Using the scaling $\Re_\infty = \Pe_\infty = 1$, the continuity, momentum, and energy equations at order \tcr{1 ??? leading order?} yield:
\begin{subequations}
...
\, .
\end{equation}
where $M$ denoted the \emph{local} Mach number. 

Once we have deduced the scaling of the numerical non-dimensionalized number, the normalization parameters $\norm_P^\mu$ and $\norm_P^\kappa$ can be derived. For brevity, only the steps leading to the derivation of $\norm_P^\kappa$ are provided; the algebra for $\norm_P^\mu$ is similar. Using the definition of the viscosity coefficients given in \eqt{eq:visc_definition} and the scaling of \eqt{eq:norm_param}, it can be shown that:
%
\begin{equation}
...
\end{equation}
%
where $\norm_{P,\infty}$ is the reference quantity for the normalization parameter $n_P$. Substituting \eqt{eq:norm_relation} into \eqt{eq:ref_numb} and recalling that the numerical Prandtl number scales as unity, we obtain:
%
\begin{equation}
...
\end{equation}
%
\eqt{eq:norm_relation_bis} provides the proper scaling for the $\kappa$ viscosity coefficient in the asymptotic limit: the normalization parameter $\norm_P^\kappa$ should scale as $\rho_\infty c_\infty^2$, which leaves us with two options:
...
\end{equation}
%
For clarity purpose, the definition of the viscosity coefficients $\mu$ and $\kappa$ are now recalled: \tcr{wrong}
%
\begin{equation}
...
\end{equation}
%
where $\norm_P^\kappa$ is computed from \eqt{eq:norm_ent2}. The jump $J$ is a function of the jump of pressure \tcr{and density gradients} across the face with respect to its normal vector $\vec{n}$. Then, the largest value over all faces is determined and used in the definition of the viscosity coefficients.
...
\end{equation}
%
where the parameters $q$ and $P_\infty$ are fluid-dependent. The values we employed are given in \tbl{tbl:stff_gas_eos}. The ideal gas equation of state is recovered by setting $q=P_\infty=0$ in \eqt{eq:eos}. 
%
\begin{table}[!htbp]
\begin{center}
