...
\begin{eqnarray}\label{eq:asympt_equ2_cont}
 ...
 &&\partial_t(\rho E)_0 + \div \left[ \vec{u} (\rho E + P) \right]_0 = \div(\kappa \grad(\rho e))_0
  \end{eqnarray}
 \end{subequations}
 Using the leading-order of the equation of state given in \eqt{eq:euler_eq2_eos}, the energy equation can be recast as a function of the leading-order pressure as follows:
...
\end{itemize}
The study of the isentropic and the non-isentropic compressible Euler equations lead to two different scaling of the non-dimensionalized numbers. In order to have a s stabilization method valid for a wide range of Mach numbers, these two choices must be merged in a single one.   
