...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\tcr{I need to continue working on the Introduction}
...
%
\tcr{I added $\| \vec{u} \|$ below. The units between the residual and the jumps were not the same. Check} \tcb{it is correct}
\begin{equation}
\label{eq:jump_CFEM}
J^K[s](t) = \max_{f\in\partial K}  \max_{\vec{r}_q \in f} \| \vec{u}(\vec{r}_q,t) \| \jmp{\grad s(\vec{r}_q,t) \cdot \vec{n}(\vec{r}_q) }_f \, ,
\end{equation}
%
...
low-Mach as was suggested during your defense. If I recall correctly, we said } \tcb{ I still have to do this}
...
%
Finally, a strong shock for a liquid phase \tcr{(Mach number around 0.1)}\tcr{I added this info. Check if you agree} \tcb{it is correct} is also performed \cite{abgrall}.
...
supersonic \tcr{you had transonic here but I think this problem is 100\% supersonic} \tcb{it is probably better to use supersonic instead of transonic since the inlet is supersonic} flow over a compression corner are provided to illustrate the ability of the new viscosity 
...
Gauss quadrature rule. Steady-state is detected in a transient simulation by monitoring the rate of change of the
conservative variables. \tcr{I have removed your portion about the total residual falling below $10^{-6}$: 
you don't clearly define total residual. Since we are implicit and thus have nonlinear residual in Newton solves, a 
reader may think you're talking about that residual. I think you wanted to say norm of the steady state residual, but
in this case we still need to define it. I decided to say something simpler, rate of change, that means $\frac{u^{n+1}-u^n}{\Delta t}$
but this may be wrong as well. It depends on how the steady-state residual are defined. Are they multiplied by $\Delta t$ or not. Please advise\\} \tcb{The stt residual is not multiplied by $\Delta t$ if I remember correctly. I monitored the norm of the total residual (time dependent and stt terms) and started with a residual of the order of $10$}
...
\end{table}
%
The entropy function for the stiffened gas equation of state is concave \tcr{am I tired or should this be concave?} \tcb{done} and given by
%
\begin{equation*}
...
\tcb{I only did convergence tests using the exact solution. I modified the formula. I do not know what happened but this is not the one I used.}
%---------------------------------------------------------------------------------------------------
...
%and thus isentropic 
\tcr{should we keep isentropic here? maybe} \tcb{done}. A convergence study was performed using the exact solution as a reference: the L$_1$ and L$_2$ norms of the 
...
%The numerical results obtained for a flow over a 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\tcr{what is an isentropic EoS? Do you want to say entropy residual formulation with a barotropic fluid instead?} \tcb{done}
...
\end{equation}
\tcr{check everywhere in this section. I have put the material derivative sign where you only had $d/dt$.} \tcb{looks good}
%
