...
% 
\begin{equation}
\partial_t \left( \rho E A \right) + \div \left[ \vec{u} A \left( \rho E + P \right) \right] = \div (\vec{h}+ \vec{u} \cdot\mathbb{g})  \,.
\end{equation}
\end{subequations}
%
...
A \rho \left( \partial_t s + \vec{u} \cdot \grad s \right) = s_e \left[ \div\vec{h}+\mathbb{g}: \grad u + \left( \frac{u^2}{2}-e \right) \div\vec{f}\right] 
...
and $\vec{h} = \vec{\tilde{h}} - 0.5 \| \vec{u} \|^2 f$ to give
%
\begin{equation}
\label{eq:ent_res_app2}
A \rho \left( \partial_t s + \vec{u} \cdot \div s \right) = s_e \left[ \div \vec{\tilde{h}}-e \div\vec{f}\right] + \rho s_{\rho} \div\vec{f} + A s_e \mu \grad \vec{u}^s : \grad \vec{u} \,. 
\end{equation}
%
...
-\div \vec{\tilde{h}} \grad s_e  +\vec{f}\cdot \grad (e s_e) - \vec{f}\cdot \grad ( \rho s_{\rho} ) + A s_e \mu \grad^s \vec{u} : \grad \vec{u} 
\end{multline}
%
...
\end{equation}
%
The first term in $rhs$ is a conservative term. By carefully choosing a definition for $\vec{\tilde{h}}$ and $\vec{f}$, the 
