...
%
When using the SGEOS, the coefficient of proportionality $\frac{s_e}{P_e}$ can be recast under the form $\frac{s_e}{P_e} = \frac{C_v ( \gamma-1)}{P + \gamma P_\infty}$ which remains positive. Note that the definition of the speed of sound can be used to show that $\resinew$ is zero when the flow becomes isentropic. The entropy residuals $\resi$ and $\resinew$ are proportional to one another and will experience similar 
...
\end{subequations}
%
Note that now the jump operator $J^K$ acts on the variables appearing in $\resinew$, namely, pressure and density. The residual $\resinew$ and the pressure jump will only contribute to the viscosity coefficients in the shock region. On the other hand, the density jump will contribute both in contact and shock regions. 
