...
and correctly resolve the discontinuities \cite{lax-peter, harten-hyman}. These numerical methods can be typified into two categories. The first category consists of artificial viscosity methods and Riemann-based solvers and is widely described in the literature \cite{van-leer, neumann, Lapidus_paper, LMP, Lapidus_book, PBV_book, Toro}. The second category relies on flux limiters techniques that takes its origin to the paper by Zalesak \cite{fct-zalesak}. 
...
The numerical method investigated in this paper belongs to the former type and is referred to as the Entropy-Viscosity Method  in the literature \cite{jlg1, jlg2}: it is a viscous regularization technique that satisfies the entropy minimum principle; adequate dissipation terms (viscous fluxes)
are added to the all of the governing laws while ensuring the entropy minimum principle still holds. It somehow shares some common ground with the hyper-viscosity method that also adds artificial dissipative terms but only to the momentum and energy equations \cite{cabot, fiorani}.
...
stabilize the numerical scheme. Values of the viscosity coefficients below $10^{-5}$ are not plotted.
%
\begin{figure}[H]
        \centering
...
The right-hand side of \eqt{eq:ent_res_app} can be simplified using the relations $\mathbb{g} = A \mu \grad^s \vec{u} +\vec{f}\otimes \vec{u}$ 
