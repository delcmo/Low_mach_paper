...
oscillations can occur with high order spatial discretization schemes due to shocks, discontinuities, or sharp changes in the solution and thus,
adequate numerical techniques are required to stabilize the numerical solution
and correctly resolve discontinuities \cite{lax-peter, harten-hyman} \tcr{should these reference stay here or go with the previous sentence?}. \tcb{we should merge them into one sentence and add the references at the end}
%
...
\tcb{
A variety of discretization schemes have been developed and employed to solve hyperbolic conservation laws. These include, but are not limited to,
total variation diminishing (TVD) schemes, weighted essentially non-oscillatory  (WENO) schemes, exact (Godunov) and approximate Riemann solvers, flux-corrected transport (FCT) schemes, artificial diffusion methods, etc.. Essentially non-oscillatory schemes and Riemann solvers differ in the reconstruction technique and the choice of numerical 
...
techniques explicitly introduce diffusion to stabilize numerical schemes, often in the form of a Laplacian operator acting on the solution variables with a viscosity coefficient that depends on the grid size and the wave speeds. This idea can be traced back to Von Neumann and Richtmyer \cite{VonNeumannRichtmyer}. Recently, Reisner et al. \cite{Reisner2013} and Barter and Darmofal \cite{darmofal2010} proposed PDE-based artificial viscosity methods, whereby an additional scalar PDE equations was coupled to the 
system of conservation laws and whose solution was the local viscosity coefficient. 
...
depends on the local entropy production; their viscous regularization method is constructed so as to satisfy the entropy minimum principle. 
...
are added to the all of the governing laws while ensuring the entropy minimum principle still holds. 
}
...
values (published with only two significant digits); see \tbl{tbl:corner_exact_sol}. The numerical results in excellent 
...
% 
\begin{equation}
\partial_t \left( \rho E A \right) + \div \left[ \vec{u} A \left( \rho E + P \right) \right] = \div (\vec{h}+ \tcr{\mathbb{g} \cdot \vec{u}} )  \,.
\end{equation}
\end{subequations}
%
...
A \rho \left( \partial_t s + \vec{u} \cdot \grad s \right) = s_e \left[ \div\vec{h} + \mathbb{g}: \grad \tcr{\vec{u}} + \left( \frac{u^2}{2}-e \right) \div\vec{f}\right] 
...
The right-hand side of \eqt{eq:ent_res_app} can be simplified using the relations \tcr{($\rho$ was missing in  $\mathbb{g}$)}
$\mathbb{g} = A \tcr{\rho} \mu \grad^s \vec{u} +\vec{f}\otimes \vec{u}$ 
and $\vec{h} = \vec{\tilde{h}} -\tfrac{1}{2} \| \vec{u} \|^2 \tcr{\vec{f}}$ to give
%
\begin{equation}
...
- \tcr{ \vec{\tilde{h}} \cdot \grad s_e}  + \vec{f}\cdot \grad (e s_e) - \vec{f}\cdot \grad ( \rho s_{\rho} ) + A s_e \mu \grad^s \vec{u} : \grad \vec{u} 
\end{multline}
%
