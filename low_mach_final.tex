...
%
%\tcr{what is below is likely what the reviewer didn't like and found pedestrian":\\
%These numerical methods can be typified into two categories. The first category consists of artificial viscosity methods and Riemann-based solvers and is widely described in the literature \cite{van-leer, neumann, Lapidus_paper, LMP, Lapidus_book, PBV_book, Toro}. The second category relies on flux limiters techniques that takes its origin to the paper by Zalesak \cite{fct-zalesak}.\\
%I am trying to change this, below. What do you think?\\}
%\tcb{
...
%
%A wide range of stabilization methods exists and can be classified into two main categories: approximate Riemann or Godunov type solvers \cite{Toro}, flux-limiter techniques \tcr{I do not consider FCT to be a stab. metehod}
%\cite{FluxLimiter, FluxLimiter2, fct-zalesak, fct-kuzmin-lohner}, and artificial viscosity methods \cite{neumann} such as pressure-based viscosity methods \cite{PBV_book} and Lapidus method \cite{Lapidus_paper, LMP, Lapidus_book}, among others. 
...
to produce a monotonic increase through the viscous layer with as little smearing as possible, but with entropy production matching that of the true shock.
...
entropy principle by transforming the Lagrangian statement above into that of an Eulerian reference frame.
Tadmor [appl. num. math. 2 (1986) 211-219] gives an alternative statement (more useful for constructing numerical methods and proofs) of the entropy minimum 
principle by transforming the Lagrangian statement above into that of an Eulerian reference frame.  Further discussion may be found in Guermond and Popov \cite{jlg}.
}
%%
...
From the oblique shock theory, an analytical solution for the ratios of 
the pre- and post-shock values for pressure, entropy and Mach number ratios are available \cite{nasa1135,CompressionCorner}. 
...
is used. The ratios for pressure, entropy, and Mach number computed suing the numerical solution are compared with the analytical 
values (published with only two significant digits); see \tbl{tbl:corner_exact_sol}. The numerical results are in excellent 
agreement with the known values. The shock wave angle at steady state can also be derived analytical and is 
...
For a Mach of 2.5 and a 15$^\circ$-corner angle, the analytical value for the shock-wave angle is $36.94^\circ$ 
