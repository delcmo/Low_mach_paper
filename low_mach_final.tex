...
shock calculations, accuracy and solution convergence in the low-Mach regime. Because of the hyperbolic nature of the equations, 
...
A wide range of stabilization methods were developed through the years and can be distinguished into three main types: approximate Riemann or Godunov type solvers \cite{Toro}, flux-limiter techniques 
\cite{FluxLimiter, FluxLimiter2, fct-zalesak, fct-kuzmin-lohner}, and artificial viscosity methods \cite{neumann} such as pressure-based viscosity methods \cite{PBV_book} and Lapidus method \cite{Lapidus_paper, LMP, Lapidus_book}, among others. The numerical method investigated in this paper belongs to the later type and is referred to as the Entropy-Viscosity Method \cite{jlg1, jlg2} in the literature.
...
becomes equivalent to a local Lax-Friedrichs scheme \cite{lf} when the first-order coefficients are employed. The local Lax-Friedrichs 
...
much smaller elsewhere, hence avoiding the over-dissipation of the local Lax-Friedrichs.  The first-order 
...
especially for general equations of state. However, this approach can be avoided by noting that the entropy 
...
of the entropy residual, whose derivation is given in \app{app:ent_res}, is the following:
%
\begin{equation}
\label{eq:ent_res}
...
$\resinew$ and the pressure jump will only contribute to the viscosity coefficients in a shock region. On the other hand, the density jump will contribute in both contact and shock regions. 
The $\mu$ and $\kappa$ coefficients are kinematic viscosities (units of $m^2/s$); the normalization 
parameters ``$\norm_P$'' are thus in units of pressure, hence the use of the subscript $P$.  We emphasize that the same normalization is not required for both $\mu_e$ and $\kappa_e$ and thus the entropy viscosity 
coefficients may be different. The low-Mach asymptotic study presented next will determine the proper normalization.
...
form'' of the Euler equations, where the conservative variables are now multiplied by the area $A$. 
...
Jacobian matrix are approximated by lagging the viscosity coefficients (computing them with the solution from the previous time). 
...
liquid water phase in \sct{sec:liquid_nozzle} because of the liquid to gas density ratio is about $1,000$ and the compressibilities are much different. An exact 
...
Subsonic 2-D hump flow is another example of an internal flow configuration. It consists of a channel of height $L=1$ $m$ 
