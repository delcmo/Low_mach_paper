...
When solving Euler equations for a wide range of Mach numbers, several questions must be addressed: stability of the numerical scheme, 
the ability to capture and track shocks, accuracy and solution convergence in the low-Mach regime. 
%
It is well known that, even with smooth initial data, the existence of a 
globally smooth solution to hyperbolic conservative laws may not occur because of the nonlinearity of the flux functions. Spurious
oscillations can occur with high order spatial discretization schemes due to shocks, discontinuities, or sharp changes in the solution. Thus,
adequate numerical techniques are required to stabilize the numerical solution
and correctly resolve discontinuities \cite{lax-peter, harten-hyman}.
%
 
%
A variety of discretization schemes have been developed and employed to solve hyperbolic conservation laws. These include 
total variation diminishing (TVD) schemes, weighted essentially non-oscillatory  (WENO) schemes, exact and approximate Riemann solvers with Godunov's method, flux-corrected transport (FCT) schemes, artificial diffusion methods, etc.. Essentially non-oscillatory schemes and Riemann solvers differ in the reconstruction technique and the choice of numerical 
fluxes employed to evolve cell-averaged quantities \cite{van-leer, neumann, Lapidus_paper, LMP, Lapidus_book, PBV_book, Toro}.  
% 
In order to ensure uniqueness of the weak (numerical) solution, an ``entropy condition'' is imposed. In practice, this is achieved
by adding an appropriate amount of dissipation (artificial viscosity), either implied by the discretization or explicitly. Artificial viscosity 
techniques explicitly introduce diffusion to stabilize numerical schemes, often in the form of a Laplacian operator acting on the solution variables with a viscosity coefficient that depends on the grid size and the wave speeds. This idea can be traced back to Von Neumann and Richtmyer \cite{VonNeumannRichtmyer}. Recently, Reisner et al. \cite{Reisner2013} and Barter and Darmofal \cite{darmofal2010} proposed PDE-based artificial viscosity methods, whereby an additional scalar PDE equation is coupled to the 
system of conservation laws and whose solution serves as the local viscosity coefficient. 
Guermond et al. \cite{jlg1, jlg2} have developed the entropy viscosity method, an entropy residual-based artificial viscosity method where the viscosity coefficient 
depends on the local entropy production; their viscous regularization method is constructed to satisfy the minimum entropy principle. 
Recently, the entropy viscisoty method method has been used as an LES model \cite{guermond_thompson}. 
Hyper-viscosity techniques also fall in the category of artificial viscosity methods and have been applied to shock-turbulence stabilization \cite{cabot, fiorani}.
%
In this paper, we improve upon the Entropy Viscosity Method (EVM) by extending it to low-Mach flows while keeping its shock-capturing properties 
for transonic and supersonic flows. In the EVM, adequate dissipation terms (viscous fluxes)
are added to all of the governing laws while ensuring the minimum entropy principle still holds. 
%%
The minimum entropy principle states that the entropy of a material particle in a flow field is constant along its pathline
...
Numerical methods are usually developed using simple equations 
...
over a broad range of Mach numbers, including in the low-Mach regime. 
...
minimum entropy principle. However, it has not been evaluated in the low-Mach regime.
...
the viscous dissipative terms are adapted so that the minimum entropy principle remains satisfied.
...
in a compression corner. % \cite{CompressionCorner} \tcr{why cite 1. cite them all, or none. 
Convergence studies are performed in 1-D and 2-D in order to demonstrate the accuracy of the solution technique for smooth 
...
recalled. The method consists of adding dissipative terms with a viscosity coefficient controlled by 
...
with the minimum entropy principle; details and proofs of the derivation can be found in \cite{jlg}. 
...
\end{subequations}
%
where $\kappa$ and $\mu$ are positive viscosity coefficients (in units of length$^2$/time). $\grad^s $ denotes the symmetric 
gradient operator and  guarantees the method to be rotationally invariant \cite{jlg} ($\grad^s \vec{u}$ represents the rate of deformation). 
The viscosity 
...
\end{equation}
%
In the current version of the method \cite{jlg1,jlg2}, the ratio of $\kappa$ to $\mu$ is defined through a numerical 
...
%, and will be further referred to as the inter-element jump. 
...
Currently, there are no theoretical justifications for choosing the denominator beyond a dimensionality argument \cite{jlg1,jlg2}. 
...
are discussed next. %in \sct{sec:new_ent_prod}.
...
derive the associated dissipative terms and verify that the minimum entropy principle is still satisfied.  
...
error and the corresponding convergence rates are computed at steady state on various uniform meshes from 4 to 1024 cells.
...
stabilize the numerical scheme. Values of the viscosity not shown in \fig{fig:1d_strong_shock_visc} are 
below $10^{-5}$.
%
\begin{figure}[H]
        \centering
...
%Subsonic 2-D hump flow is another example of an internal flow configuration. 
In this example, the fluid enters the domain with a given uniform velocity and encounters an obstacle. Various
inlet Mach values are employed, from $M_{\infty}=10^{-7}$ to $M_{\infty}=0.7$.
...
In this last example, we consider a supersonic flow at Mach 2.5 impinging on a corner with an angle of $\theta=15^\circ$. 
A shock will form at an angle $\beta > \theta$. From the oblique shock theory, 
an analytical solution for pre- and post-shock states is available \cite{nasa1135,CompressionCorner} and will be employed
to perform a convergence study. 
...
temperature and velocity can be specified using Dirichlet boundary conditions. The outlet is also supersonic and none 
of the characteristics enter the domain through this boundary. %; these values are computed by the solver.

The simulation is run with $\text{CFL}=2$ until steady state is reached. A sequence of uniformly 2-D meshes, composed $\mathbb{Q}_1$ elements 
is used (48 elements at the coarsest level, 49,152 elements at the finest level). In \tbl{tbl:corner_exact_sol}, the compare the ratios for pressure, 
entropy, and density computed using both the analytical solution and the numerical solution using a mesh with 3,072 elements; 
the numerical results are in excellent agreement with the exact values. The shock wave angle at steady state can be derived analytical and is 
given by the so-called $\theta -\beta -M$ relation:
%
\begin{equation}
...
For a Mach of 2.5 and a 15$^\circ$-corner angle, the analytical value for the shock-wave angle is $36.9449^\circ$ 
...
%Pressure    &  2.46750012784148      & 2.46822899802994    \\ \hline
%Mach number &  0.74941040270010      & 0.74918036987472    \\ \hline
%Density     &  1.86654863046077      & 1.86754966717355    \\ \hline 
            & analytical & numerical\\ \hline
Pressure    &  2.4675      & 2.4682    \\ \hline
Mach number &  0.7494      & 0.7492   \\ \hline
Density     &  1.8665      & 1.8675    \\ \hline 
\end{tabular}
\caption{\label{tbl:corner_exact_sol} Ratio of analytical and numerical downstream to upstream quantities for 
the compression corner problems (corner angle of $15^\circ$ and inlet $M=2.5$ (mesh with 3,072 elements).}
\end{center}
\end{table}
%
\begin{figure}[H]
        ...
        \end{subfigure}
        \caption{Steady-state solution for a flow in a 2-D compression corner (mesh with 3,072 elements).}\label{fig:2d_corner}
\end{figure}
%
...
\fig{fig:2d_corner_visc} shows that the viscosity coefficient is large in the shock and extremely small elsewhere, 
as expected. At the location of the corner ($x=-0.25m$, $y=-0.5m$), a peak in the viscosity coefficient can be noted. 
%because of the treatment of the wall boundary condition: at this particular node, the normal is not well 
%defined and may cause some numerical errors. 
Values for pressure and Mach number are graphed in \fig{fig:2d_corner_isomach} at the location $y=0$: 
no spurious oscillations are observed and the shock is well resolved. 
The difference between the inlet and outlet mass flow rates is plotted in \fig{fig:2d_convergence} (log scale)
and shows that a steady state has indeed been reached. 

We conclude by presenting a convergence study for this test case. We employ a series of uniformly refined grids
using $\mathbb{Q}_1$ finite elements. The L$_1$ and L$_2$ norms of the error in the density are computed 
using a two-dimensional Gauss-Legendre quadrature as follows
%\be
%\| u - u_h \|_r = \left( \int_{\text{domain}} \left[u(x,y)-u_h(x,y)\right]^r dxdy \right)^{1/r} =  \left( \sum_K \sum_q w_q \left[ u(x_q,y_q)-u_h(x_q,y_q) \right]^r \right)^{1/r} 
%\ee
\begin{subequations}
\begin{align}
\| \rho - \rho_h \|_1 = \int_{\text{domain}} | \rho(x,y)-\rho_h(x,y)| dxdy =  \sum_K \sum_q w_q | \rho(x_q,y_q)-\rho_h(x_q,y_q)| 
\end{align}
\be
\| \rho - \rho_h \|_2 = \sqrt{\int_{\text{domain}} \left[\rho(x,y)-\rho_h(x,y)\right]^2 dxdy} =  \sqrt{\sum_K \sum_q w_q \left[ \rho(x_q,y_q)-\rho_h(x_q,y_q) \right]^2 } 
\ee
\end{subequations}
where $\rho$ and $\rho_h$ denote the exact and numerical density solutions, $q$ is the summation index over the quadrature points, $K$ is a mesh cell, and $w_q$ is the jacobian of the mapping from the physical cell $K$ to the reference element $[-1,1]^2$ evaluated at quadrature points $(x_q, y_q)$ and multiplied by the quadrature weight. 
When a fine-enough mesh is employed, the error is expected to be proportional to $h^p$, where $h$ is the typical mesh size and $p$ the convergence rate. In 2D, $h^2$ is proportional to the number of elements, $N_\text{el}$, so that the error is expected to be proportional to $\left(N_\text{el}\right)^{p/2}$. Since the solution contains a shock, the theoretical estimates of the convergence rates $p$ in the L$_1$ and L$_2$ norms are 1 and $1/2$, respectively.
The results of this convergence study are presented in \tbl{tbl:corner_convergence}; we observe that the convergence rates are 
1 and $1/2$ in the L$_1$ and L$_2$ norms, respectively, as expected.
%
\begin{table}[H]
\begin{center}
\begin{tabular}{|r|r|c|r|c|}  \hline
$N_\text{el}$   &  L$_1$ error   &  L$_1$ rate  &  L$_2$ error   &  L$_2$ rate  \\ \hline
     %48  &   0.219076478314102    &          -           &     0.271554019689942    &             -          \\ \hline
    %192  &   0.129061065669841    &   0.763380751017214  &     0.200809351033798    &     0.435412766538997  \\ \hline
    %768  &   0.0730967024280946   &   0.820175613878837  &     0.147764481642899    &     0.442526923924373  \\ \hline
  %3,072  &   0.0391538639846831   &   0.900651634069645  &     0.106545580588655    &      0.47182877579512  \\ \hline
 %12,288  &   0.0203278442067009   &   0.945697466375151  &     0.0758696121024894   &     0.489876685909544  \\ \hline
 %49,152  &   0.0104106275866897   &   0.965400182045967  &     0.0539588859596159   &     0.491661599122136  \\ \hline
     %48  &   219.076478314102E-03   &          -           &    271.554019689942E-03   &             -          \\ \hline
    %192  &   129.061065669841E-03   &   0.763380751017214  &    200.809351033798E-03   &     0.435412766538997  \\ \hline
    %768  &   73.0967024280946E-03   &   0.820175613878837  &    147.764481642899E-03   &     0.442526923924373  \\ \hline
  %3,072  &   39.1538639846831E-03   &   0.900651634069645  &    106.545580588655E-03   &      0.47182877579512  \\ \hline
 %12,288  &   20.3278442067009E-03   &   0.945697466375151  &    75.8696121024894E-03   &     0.489876685909544  \\ \hline
 %49,152  &   10.4106275866897E-03   &   0.965400182045967  &    53.9588859596159E-03   &     0.491661599122136  \\ \hline
     48  &   219.08E-03   &    -    &    271.55E-03   &     -     \\ \hline
    192  &   129.06E-03   &   0.76  &    200.81E-03   &     0.43  \\ \hline
    768  &    73.10E-03   &   0.82  &    147.76E-03   &     0.44  \\ \hline
  3,072  &    39.15E-03   &   0.90  &    106.55E-03   &     0.47  \\ \hline
 12,288  &    20.33E-03   &   0.95  &     75.87E-03   &     0.49  \\ \hline
 49,152  &    10.41E-03   &   0.97  &     53.96E-03   &     0.49  \\ \hline
  \end{tabular}
\caption{\label{tbl:corner_convergence} Convergence study for the compression corner test case.}
\end{center}
\end{table}
%left = 1.2222122856;
%rite = 2.281318668;
...
to correctly simulate several types of flows (from very low Mach subsonic to supersonic flows) without tuning parameters.
...
the exact solution was demonstrated by computing the convergence rates of the L$_1$ and L$_2$ norms 
...
In the future, we plan to further extend the entropy viscosity method 
to the seven-equation two-phase flow model \cite{SEM}. 
This two-phase flow system of equations is a good candidate for two reasons: it is unconditionally 
hyperbolic and degenerates to the standard Euler equations when one phase disappears.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Derivation of the dissipative terms for the Euler equations with variable area using the minimum entropy principle} \label{app:diss_terms}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
\begin{equation}
\partial_t \left( \rho A \right) + \div \left( \rho \vec{u} A \right) = \div \vec{f} 
\end{equation}
%
\begin{equation}
\partial_t \left( \rho \vec{u} A \right) + \div \left[A\left( \rho \vec{u} \otimes \vec{u} + P \mathbb{I} \right) \right] = P \grad A + \div\mathbb{g}
\end{equation}
% 
\begin{equation}
\partial_t \left( \rho E A \right) + \div \left[ \vec{u} A \left( \rho E + P \right) \right] = \div (\vec{h}+ \mathbb{g} \cdot \vec{u} )  \,.
\end{equation}
\end{subequations}
%
where $\vec{f}$, $\mathbb{g}$ and $\vec{h}$ are dissipative fluxes yet to be determined. Starting from the modified system of equations 
...
A \rho \left( \partial_t s + \vec{u} \cdot \grad s \right) = s_e \left[ \div\vec{h} + \mathbb{g}: \grad vec{u} + \left( \frac{u^2}{2}-e \right) \div\vec{f}\right] 
+ \rho s_{\rho} \div\vec{f}\,. 
\end{equation}
%
The next step consists of choosing a definition for each of the dissipative terms so that the left hand-side is positive. 
The right-hand side of \eqt{eq:ent_res_app} can be simplified using the relations
$\mathbb{g} = A \rho \mu \grad^s \vec{u} +\vec{f}\otimes \vec{u}$ 
and $\vec{h} = \vec{\tilde{h}} -\tfrac{1}{2} \| \vec{u} \|^2 \vec{f}$ to give
%
\begin{equation}
\label{eq:ent_res_app2}
A \rho \left( \partial_t s + \vec{u} \cdot \div s \right) = s_e \left[ \div \vec{\tilde{h}}-e \div\vec{f}\right] + \rho s_{\rho} \div\vec{f} + A s_e \mu \grad^s \vec{u} : \grad \vec{u} \,. 
\end{equation}
%
where $\grad^s$ is the symmetric gradient. 
The right-hand side is now integrated by parts:
%
\begin{multline}
\label{eq:ent_res_app3}
A \rho \left( \partial_t s + \vec{u} \cdot \div s \right) = \div \left[ s_e \vec{\tilde{h}}-s_e e\vec{f} + \rho s_{\rho}\vec{f}\right]  \\
- \vec{\tilde{h} \cdot \grad s_e}  + \vec{f}\cdot \grad (e s_e) - \vec{f}\cdot \grad ( \rho s_{\rho} ) + A s_e \mu \grad^s \vec{u} : \grad \vec{u} 
\end{multline}
%
The term $A s_e \mu \grad^s \vec{u} : \grad \vec{u}$ is positive and thus, 
does not need any further manipulations. It remains to treat the other terms of the right-hand side that we now call $rhs$:
%
\begin{equation}
rhs = \div \left[ s_e \vec{\tilde{h}}-s_e e\vec{f} + \rho s_{\rho}\vec{f}\right] - \vec{\tilde{h}} \cdot \grad s_e  +\vec{f}\cdot \grad (e s_e) -\vec{f}\cdot \grad ( \rho s_{\rho} )  \,. \nonumber
\end{equation}
%
The first term in $rhs$ is a conservative term. By carefully choosing a definition for $\vec{\tilde{h}}$ and $\vec{f}$, this 
...
equations are recovered. The following definitions for $\vec{\tilde{h}}$ and $\vec{f}$ are chosen:
%
\begin{equation}
\vec{\tilde{h}} = A \kappa \grad ( \rho e ) \text{  and  }\vec{f}= A \kappa \grad \rho, \nonumber 
\end{equation}
%
...
\end{equation}
%
We now deal with the term $\mathbf{Q}$ and note it can be recast as a quadratic form. One obtains:
%
\begin{eqnarray}
...
can be simply determined by studying the positiveness of matrix $\Sigma$. In this particular case, we have 
to prove that this matrix is negative definite since the quadratic form is on the right-hand side of the residual expression and it is preceded by a negative 
...
and the specific volume $1/ \rho$ is sufficient to ensure that matrix $\Sigma$ is negative definite. \\
Finally, the right-hand side of the entropy residual \eqt{eq:ent_res_app} has now been expressed as the sum of a conservative term and another term known to be positive. Thus, the minimum entropy principle holds.
